...
func dashboard(status, region string, params DashParams, data ...grafana.GetDetails) grafana.GetDetails {
    ...
    for i,_ := range panels {
        if panels[i].Targets != nil {
            if fmt.Sprintf("%d", panels[i].ID) == params.AsgPanel {
                fmt.Println(panels[i].Targets[0].Dimensions.AutoScalingGroupName)
            }
        }
    }
